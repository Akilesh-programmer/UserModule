...
export class CountryService {
  ...
  async findOne(id: string) {
    ...
    if (!doc) throw new RpcException({ statusCode: 404, message: 'Country not found' });
    return doc;
  }

  async create(dto: { name: string; code?: string; isActive?: boolean }) {
    const exists = await this.model.findOne({ name: new RegExp(`^${dto.name}$`, 'i') }).exec();
    if (exists) throw new RpcException({ statusCode: 409, message: `Country '${dto.name}' already exists` });
    return this.model.create(dto);
  }

  async update(id: string, dto: Partial<{ name: string; code: string; isActive: boolean }>) {
    if (dto.name) {

...
export const fetchActiveStates = (countryId) => {
  const params = countryId ? `?countryId=${countryId}` : "";
  return api.get(`/states/active${params}`).then(unwrap);
};
export const fetchStatesByCountry = (countryId) => api.get(`/states/by-country/${countryId}`).then(unwrap);

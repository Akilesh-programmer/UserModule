...
import { ShopType, ShopTypeDocument } from './schemas/shop-type.schema';

@Injectable()
export class ShopTypeService {
  constructor(@InjectModel(ShopType.name) private readonly model: Model<ShopTypeDocument>) {}
  ...
  async findOne(id: string) {
    ...
    if (!doc) throw new RpcException({ statusCode: 404, message: 'Shop type not found' });
    return doc;
  }
  ...
  async create(dto: { name: string; description?: string; isActive?: boolean }) {
    ...
    if (exists) throw new RpcException({ statusCode: 409, message: `Shop type '${dto.name}' already exists` });
  ...
  async delete(id: string) {
    ...
    if (!doc) throw new RpcException({ statusCode: 404, message: 'Shop type not found' });
    return { message: 'Shop type deleted successfully' };
  }
}

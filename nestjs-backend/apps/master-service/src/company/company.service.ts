...
export class CompanyService {
  ...
  async findOne(id: string) {
    ...
    if (!doc) throw new RpcException({ statusCode: 404, message: 'Company not found' });
    return doc;
  }

  async create(dto: any) {
    const exists = await this.model.findOne({ name: new RegExp(`^${dto.name}$`, 'i') }).exec();
    if (exists) throw new RpcException({ statusCode: 409, message: `Company '${dto.name}' already exists` });

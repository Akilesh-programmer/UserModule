...
import { ExpenseType, ExpenseTypeDocument } from './schemas/expense-type.schema';

@Injectable()
export class ExpenseTypeService {
  constructor(@InjectModel(ExpenseType.name) private readonly model: Model<ExpenseTypeDocument>) {}
  ...
  async findOne(id: string) {
    ...
    if (!doc) throw new RpcException({ statusCode: 404, message: 'Expense type not found' });
    return doc;
  }
  ...
  async create(dto: { name: string; description?: string; isActive?: boolean }) {
    ...
    if (exists) throw new RpcException({ statusCode: 409, message: `Expense type '${dto.name}' already exists` });
